...
var Simulation = {
    ...
    load: function() {
        ...
        $(".topnav section").css("color", "white");
    ...
    start: function() {
        ...
        if ($('#mapSelection').val() == null) {
            alert("Choose IATA code and Simulation file.");
            console.log($('#simSelection').val());
        } else if ($('#simSelection').val() == null) {
            alert("Choose Simulation file.");
        } else {
    ...
    changeSimSpeed: function() {
        //sets new interval for framerate
        clearInterval(this.interval)
        var multiplier = $("#myRange").val();
...
function Flight(json) {
    //color, x, y, degrees, speed, ID, altitude,type) {
    this.frameAge = 0;
    ...
    this.targetDirection = parseFloat(json.targetDirection);
    ...
    this.toggleDisp = function() {
        // controller for data block toggling
        this.toggler = !this.toggler;
    }

    this.move = function() {
        /// calculates (x,y) position for next frame
        ...
        this.x += (this.speed / 1000) * (Math.sin(bearing));

        this.y += ((this.speed / 1000) * (Math.cos(bearing))) * -1;
    ...
    this.seekSpeed = function() {
        ...
        if (this.speed.toFixed(2) < this.targetSpeed.toFixed(2)) {
            if (Math.abs(this.speed - this.targetSpeed) <= 5) {
                delta = 1;
            }
            this.speed += delta; // amount of change per frame
    ...
    this.update = function() {
        /// draws flight icon and info onto screen
        if (this.active) {
            ...
            let speed = ("00000" + String(this.speed)).slice(-3);
            if (!this.toggler) {
                if (toggle2) {
...
function loadPage() {
    ...
    .done(function(json) {
        All_Flight_Sims = []; //json['orig'];
        mapsListTEST = json['maps'];
        angular.element($('body')).scope().Sims_List = All_Flight_Sims;
        angular.element($('body')).scope().mapList = mapsListTEST;
        angular.element($('body')).scope().$apply();
...
//console.log('mapOBJs ', mapsListTEST);
...
function loadSim() {
    ...
    flightsComing = [];
    flights = [];
    flightListJSON = angular.element($('body')).scope().Sims_List[$("#simSelection").val()]; /// default Sim
    // console.log(flightListJSON);
    flights = [];
    //console.log(flightListJSON);
    for (flight in flightListJSON) {
        // loading flight objects from json into array
...
function changeMap(obj) {
    ...
    setTimeout(function() {
        ...
        angular.element($('body')).scope().Sims_List = coordinates["sims"];
        angular.element($('body')).scope().$apply();
        $("#mapBlocker").text("");
        map.setCenter({
